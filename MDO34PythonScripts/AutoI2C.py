#Majid Reza Barghi 
#This code is used for automatically collecting data and using the google API to upload the data on to a google sheet 
...
""")
    ...
    EndRowValue = int(NextRowValue)+16
...
""")
    ...
    imgFile.write(imgData)
    imgFile.close()
    DataSetupTimeUnits = scope.query("CURSor:HBArs:UNIts?")
    DataSetupTime = scope.query("CURSor:HBArs:DELTa? ")
    print(DataSetupTime)
...
""")
    ...
    imgFile.write(imgData)
    imgFile.close()
    DataHoldTimeUnits = scope.query("CURSor:HBArs:UNIts?")
    print(DataHoldTime)
    DataHoldTime = scope.query("CURSor:HBArs:DELTa? ")
